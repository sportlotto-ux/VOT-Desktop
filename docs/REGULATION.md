...
**Обновления (как Parabolic, src-tauri/src/updates.rs):** при запуске (не чаще 1×/сутки через маркер `last-update-check`) запрашивается GitHub `releases/latest` для yt-dlp и deno. Если версия новее используемой (PATH/кэш) — UI показывает «yt-dlp: vX → vY [Update]». Кнопка вызывает `update_binary`, который качает latest и проверяет sha256 **из самого релиза** (`SHA2-256SUMS` для yt-dlp, `*.sha256sum` для deno) — pin-константы для кэша не нужны. Автоскачивания нет: обновление инициирует пользователь.

**Причина:** выпуск нового yt-dlp/deno не должен требовать пересборки AppImage. Пользователь обновляет системный пакет — приложение подхватывает его; если его нет — приложение ставит pinned версию в кэш, а свежие версии предлагает установить через кнопку.
...
- Автообновление без подтверждения — отвергнуто: пользователь решает, когда качать (как Parabolic)

**Последствия:**
- AppImage: 88MB (было 156MB), .deb: 2.3MB
- Первый запуск без deno/yt-dlp требует сеть + ~10-30s (скачивание + sha256)
- GitHub API вызывается максимум 1×/сутки; при сетевой ошибке — тихий отказ (пустой список)
- Проверка обновлений — только для кэша/PATH, системный пакет обновляется пакетным менеджером
- `bundle.resources` в tauri.conf.json пуст; `build-appimage.sh` без fetch; сетевые тесты — `#[ignore]`
...
| 2.1 | 2026-08-30 | Фикс «зависания» лога: stderr yt-dlp стримится в UI (ретраи/ошибки видны сразу); VOT: fallback для 'Translation not available' (vot-cli-live 1.7.5), heartbeat, стриминг stderr, таймаут 300→600с |
| 2.2 | 2026-08-30 | Критический фикс: аудио-загрузка с включённой озвучкой писалась во временную .vot-process-* папку и удалялась после «успеха» — теперь work-dir только для микса; чекбокс озвучки блокируется для Audio; live-регрессионный тест |
| 2.3 | 2026-08-30 | Микширование как единый этап для видео И аудио: галочка «Микшировать с русской озвучкой» теперь работает для аудио-файлов (оригинал + VOT-озвучка → .mixed.m4a), unified pipeline вместо разрозненных веток, чекбокс снова активен для Audio |
